...
class TrajectoryData(SingleData):
    ...
    def get(self, traj_index, sub_traj_index, backend: ModuleType = jnp):
        ...
        return SingleData(
            qpos=backend.squeeze(self.qpos[ind].copy().astype(backend.float32)),
            qvel=backend.squeeze(self.qvel[ind].copy().astype(backend.float32)),
            xpos=backend.squeeze(self.xpos[ind].copy().astype(backend.float32)) if self.xpos.size > 0 else backend.empty((1, 0)),
            xquat=backend.squeeze(self.xquat[ind].copy().astype(backend.float32)) if self.xquat.size > 0 else backend.empty((1, 0)),
            cvel=backend.squeeze(self.cvel[ind].copy().astype(backend.float32)) if self.cvel.size > 0 else backend.empty((1, 0)),
            subtree_com=backend.squeeze(self.subtree_com[ind].copy().astype(backend.float32)) if self.subtree_com.size > 0 else backend.empty((1, 0)),
            site_xpos=backend.squeeze(self.site_xpos[ind].copy().astype(backend.float32)) if self.site_xpos.size > 0 else backend.empty((1, 0)),
            site_xmat=backend.squeeze(self.site_xmat[ind].copy().astype(backend.float32)) if self.site_xmat.size > 0 else backend.empty((1, 0))
        )
    ...
    def get_fast(self, traj_index, sub_traj_index, backend: ModuleType = jnp):
        ...
        return backend.squeeze(self.qpos[ind].copy().astype(backend.float32)), backend.squeeze(self.qvel[ind].copy().astype(backend.float32))
